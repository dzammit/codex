...
const USER_MESSAGE_PREFIX: &str = "user › ";
const USER_MESSAGE_PREFIX_COLS: u16 = 7;
...
fn prefix_user_message_lines(lines: Vec<Line<'static>>) -> Vec<Line<'static>> {
    lines
        .into_iter()
        .enumerate()
        .map(|(index, line)| {
            if index > 0 && line.spans.iter().all(|span| span.content.trim().is_empty()) {
                return Line::from("").style(line.style);
            }

            let prefix = if index == 0 {
                USER_MESSAGE_PREFIX.cyan().bold()
            } else {
                "       ".into()
            };
...
impl HistoryCell for UserHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.extend(prefix_user_message_lines(content_lines));
...
mod tests {
    ...
    fn user_history_cell_wraps_and_prefixes_each_line_snapshot() {
        ...
        // Small width to force wrapping while leaving room for the user label.
        let width: u16 = 22;
        let lines = cell.display_lines(width);
        let rendered = render_lines(&lines).join("\n");
